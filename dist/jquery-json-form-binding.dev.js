    ...
    $.each(options.data, function (k, v) {
      ...
      if (options.callbacks != null && options.callbacks.hasOwnProperty(k)) {
        ...
        $(elements).each(function (index, ele) {
          if (Array.isArray(v)) {
            // checkbox group and multiple select will have array of values
            v.forEach(function (val) {
              $(ele).is("select") // check if select control
              ? $(ele).find("[value='" + val + "']").prop("selected", true) // select option if select control
              : $(ele).val() == val ? $(ele).prop("checked", true) // mark as checked if radio or checkbox
              : "";
            });
